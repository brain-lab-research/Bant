...
class RecessServer(Server):
    def __init__(self, cfg, baseline_decreased_score, init_trust_score):
        super().__init__(cfg)
        self.trust_scores = [
            init_trust_score for _ in range(cfg.federated_params.amount_of_clients)
        ]
    ...
    def calculate_trust_scores(self) -> None:
        for rank in range(len(self.client_gradients)):
            abnormality_alpha = self.calculate_abnormality_alpha(rank)
            self.trust_scores[rank] = (
