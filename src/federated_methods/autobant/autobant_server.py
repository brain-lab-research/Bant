...
from .autobant_models import AutoBANTModel2d, MemoryEfficientAutoBANTModel


class AutoBANTServer(Server):
    def __init__(
        self,
        cfg,
        trust_df,
    ...
    def _init_trust_model(self):
        if self.num_clients == 10:
            self.trust_model = AutoBANTModel2d(
                self.cfg,
            ...
            self.trust_model = MemoryEfficientAutoBANTModel(
                self.cfg,
                self.global_model.state_dict(),
                self.client_gradients,
                self.device,
    ...
    def _count_trust_score(self):
        ...
        for epoch in range(self.num_opt_epochs):
            epoch_loss = 0
            for i, batch in enumerate(self.trust_loader):
                # Train step
                ...
                epoch_loss += loss
                print(f"Iteration {epoch*len(self.trust_loader) + i}: Loss {loss}")
        ...
        if self.end_point == "origin":
            for i in range(len(best_trust_scores)):
                if best_trust_scores[i] < 0.5 / self.num_clients:
