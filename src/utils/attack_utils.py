...
def apply_synchronized_attack(
    client_gradients, client_map_round, attack_configs, global_model
):
    ...
    def apply_ipm_attack(ipm_eps):
        ...
        byzantine_clients = [
            i for i, attack_type in client_map_round.items() if attack_type == "ipm"
        ]
    ...
    def apply_alie_attack(backdoor_type):
        ...
        num_attacked_clients = len(
            [v for v in client_map_round.values() if v == "alie"]
        )
        ...
        byzantine_clients = [
            i for i, attack_type in client_map_round.items() if attack_type == "alie"
        ]
        ...
        mean_grads = {
            k: byzantine_grads[k].mean(0) for k, _ in global_model.named_parameters()
        }
        std_grads = {
            k: byzantine_grads[k].std(0) for k, _ in global_model.named_parameters()
        }
